...
Native output insertion stays explicit, dimension-aware, selection-aware, and undoable. Image decoding is guarded against page/account changes. Mode and panel changes do not cancel a batch; page changes remain blocked until generation/insertion is finished or cancelled. Duplicating an essay figure preserves the measured native text geometry as well as internal bindings; the real-browser test checks the complete original scene, offset copies, and a single native Undo.
...
Release preflight on August 26: the private `swyxdotio-draw-assets` bucket was created and read back successfully; public `r2.dev` access is disabled and no custom domains are attached. This does not by itself activate the Worker binding. Wrangler authentication works for the configured Cloudflare account, and existing Git SSH authentication verifies `swyxio`; the stale `gh` token is not needed for source publication. The independent Tools task subsequently verified its `84889e5` release on main version `8cd8b896-9db2-4892-ac47-59741c8dc42e`. The pre-Draw companion is `967f4344-cac2-4155-87df-85f94cba2a5e`, with namespace `3656ef00b9ec4c4a898976749f20d2c4`. Re-read both deployments at release time; these preflight versions are not proof of the consolidated release. Final combined tests and new source/version/binding/browser verification remain required before claiming it is live.
