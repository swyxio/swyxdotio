...
Recent AI Engineer and Latent Space uploads are **context only, not recommended examples of good design**. Neither branded starter automatically previews recent or most-viewed videos; a clear reference-selection prompt replaces those recommendations. The browser initially opens Dwarkesh, and the two recent house feeds carry a visible context-only notice. All seven dated Latest/Popular lists remain available for explicit comparison. Recency and popularity never imply house approval. This presentation correction does not remove or rewrite any user's already-selected or saved examples.
