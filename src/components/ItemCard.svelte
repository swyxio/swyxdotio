	...
	<div class="w-full text-gray-900 dark:text-gray-100">
		<div class="w-full mb-8">
			{#if item.instances.length === 1}
				...
				>
					{#if item.instances[0].video}
						<img class="h-32 w-fit" alt="talk thumbnail" src={`http://i3.ytimg.com/vi/${new URL(item.instances[0].video).searchParams.get('v')}/hqdefault.jpg`} />
					{/if}
				</a>
				<div class="flex-auto justify-between md:flex-row">
			...
			{:else}
				...
				<ul class="flex flex-col items-center py-2 border-y-2 border-white my-4">
