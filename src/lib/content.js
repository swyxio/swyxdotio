...
export async function getBlogpost(slug) {
	...
	if (blogpost) {
		...
		let content = (await compile(blogpost.content, {
			remarkPlugins,
			rehypePlugins
		})).code
		content = content
...
function parseIssue(issue) {
	...
	try {
		...
		let tags = []
		if (data.tags) tags = Array.isArray(data.tags) ? data.tags : [...data.tags.split(',').map(tag => tag.trim())]
		else if (data.categories) {
			tags = Array.isArray(data.categories) ? data.categories : [...data.categories.split(',').map(tag => tag.trim())]
