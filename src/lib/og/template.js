...
export function renderNotebookTemplate(card, portrait) {
	...
	const description = card.description
		? `<p style="display:block;max-width:780px;margin:24px 0 0;color:#566273;font-size:27px;line-height:1.35">${escapeHtml(card.description)}</p>`
		: '';
	const copy = div(
		...
		`${portraitMarkup}<h1 style="display:block;max-height:${Math.ceil(effectiveTitleSize * 2.95)}px;margin:0;overflow:hidden;${FONT_TITLE}font-size:${effectiveTitleSize}px;line-height:.98;letter-spacing:-.035em">${escapeHtml(card.title)}</h1>${description}${variantDetail(card)}`
	);
	...
	const signature = div(
		...
		`${smallPortrait}${div(`${FONT_TITLE}margin-right:4px;color:#C0392B;font-size:54px;line-height:.8;`, '[')}${div(`${FONT_NOTE}color:#C0392B;font-size:31px;`, escapeHtml(card.annotation))}`
	);
	const footer = div(
		`${FLEX}align-items:flex-end;justify-content:space-between;min-height:65px;`,
		`${signature}${card.date ? div('color:#697386;font-size:21px;letter-spacing:.04em;', escapeHtml(card.date)) : ''}`
	);
	const top = div(
		`${FLEX}align-items:center;justify-content:space-between;font-size:22px;text-transform:uppercase;letter-spacing:.16em;`,
		`${div('flex-shrink:0;color:#2563EB;font-weight:600;', escapeHtml(card.label))}${div('flex-shrink:0;color:#697386;text-transform:none;letter-spacing:.05em;', 'swyx.io')}`
	);
	const paper = div(
