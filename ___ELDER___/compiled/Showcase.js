...
const Showcase = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	filteredData = data.filter(x => {
		...
		if ( x.type === "Tutorials") return true;
	});

	return `<div class="${"relative max-w-lg mx-auto lg:max-w-7xl mb-8"}"><div class="${"text-center"}"><h2 class="${"text-3xl leading-9 tracking-tight font-extrabold text-gray-900\n        sm:text-4xl sm:leading-10"}">Idea Showcase
    </h2>
    <p class="${"mt-3 text-xl leading-7 text-gray-500 sm:mt-4"}">For Free: ${escape(filteredData.length)} Great Ideas. Lightly Used.
    </p></div></div>

<div><div class="${"flex rounded-md shadow-sm mb-4"}">
    ...
    <div class="${"relative flex-grow focus-within:z-10"}"><div class="${"absolute inset-y-0 left-0 pl-3 flex items-center"}">
        ...
        <input id="${"search_candidate"}" class="${"form-input block w-full rounded-none rounded-l-md pl-2\n            transition ease-in-out duration-150 sm:hidden"}" placeholder="${"Search"}">
        <input id="${"search_candidate"}" class="${"hidden form-input w-full rounded-none rounded-l-md pl-2\n            transition ease-in-out duration-150 sm:block sm:text-sm sm:leading-5"}" placeholder="${"Search candidates"}"></div></div>

    
    <button class="${"-ml-px relative inline-flex items-center px-4 py-2 border\n        border-gray-300 text-sm leading-5 font-medium rounded-r-md text-gray-700\n        bg-gray-50 hover:text-gray-500 hover:bg-white focus:outline-none\n        focus:shadow-outline-blue focus:border-blue-300 active:bg-gray-100\n        active:text-gray-700 transition ease-in-out duration-150"}">
  ...
  <div class="${"bg-white shadow overflow-hidden sm:rounded-md"}"><ul>${each(filteredData, item => `<li><a href="${"#"}" class="${"block hover:bg-gray-50 focus:outline-none focus:bg-gray-50\n              transition duration-150 ease-in-out"}"><div class="${"px-4 py-4 sm:px-6"}"><div class="${"flex items-center justify-between"}"><div class="${"text-sm leading-5 font-medium text-indigo-600 truncate"}">${escape(item.title)}</div>
                <div class="${"ml-2 flex-shrink-0 flex"}"><span class="${"px-2 inline-flex text-xs leading-5 font-semibold\n                      rounded-full bg-green-100 text-green-800"}">${escape(item.type)}</span>
                </div></div>
              <div class="${"mt-2 sm:flex sm:justify-between"}"><div class="${"sm:flex"}"><div class="${"mr-6 flex items-center text-sm leading-5\n                      text-gray-500"}">
	...
	: "Uncategorized")}
                ...
                <div class="${"mt-2 flex text-sm leading-5 text-gray-500\n                    sm:mt-0"}">${item.type === "Talks"
	? `
                    ${each(item.instances, instance => `<div class="${"flex"}"><svg class="${"flex-shrink-0 mr-1.5 h-5 w-5 text-gray-400"}" viewBox="${"0 0 20 20"}" fill="${"currentColor"}"><path fill-rule="${"evenodd"}" d="${"M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z"}" clip-rule="${"evenodd"}"></path></svg>
                        <span><time${add_attribute("datetime", new Date(instance.date).toISOString().slice(0, 10), 0)}>${escape(new Date(instance.date).toDateString().slice(4))}</time> @ ${escape(instance.venue)}</span>
                      </div>`)}`
	: `
                      ...
                      <span><time${add_attribute("datetime", new Date(item.date).toISOString().slice(0, 10), 0)}>${escape(new Date(item.date).toDateString().slice(4))}</time>
                      </span>`}
                </div></div>
            </div></a>
        </li>`)}</ul></div></div>
